...
export function SalesReport() {
  ...
  const filterDataByDateRange = () => {
    const filtered = salesData.filter(item => {
      ...
      return itemDate >= startOfDay(dateRange.startDate) && 
             itemDate <= endOfDay(dateRange.endDate);
    });
    ...
    <div className="p-8 space-y-8">
      ...
      >
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            Showing data from {format(dateRange.startDate, 'MMM dd, yyyy')} to {format(dateRange.endDate, 'MMM dd, yyyy')}
          </span>
