...
export function TopCompanies({ data }: TopCompaniesProps) {
  ...
  const companySummaries = data.reduce((acc, item) => {
    ...
    return acc;
  }, {} as Record<string, CompanySummary & { productCount: Set<string> }>);

  const topCompanies = Object.values(companySummaries)
    .map(company => ({
      ...company,
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold">Top Contributing Companies</h2>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {company.productCount} products • {company.transactionCount} orders
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-green-600">
                    Rp {company.totalRevenue.toLocaleString('id-ID')}
                  </p>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div className="text-center p-2 rounded-lg bg-purple-50">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    ...
                    <span className="text-xs text-purple-700">Profit</span>
                  </div>
                  <p className="font-semibold text-purple-600">
                    Rp {company.totalProfit.toLocaleString('id-ID')}
                  </p>
                </div>
                <div className="text-center p-2 rounded-lg bg-orange-50">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    <span className="text-xs text-orange-700">Avg Order</span>
                  </div>
                  <p className="font-semibold text-orange-600">
                    Rp {company.averageOrderValue.toLocaleString('id-ID')}
                  </p>
                </div>
              </div>
            </motion.div>
      ...
      {topCompanies.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="text-muted-foreground">No company data available for this date range</p>
        </div>
      )}
    </motion.div>
  );
