...
export function ExportOptions({ data, dateRange }: ExportOptionsProps) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      topProducts.forEach((product: any, index) => {
        pdf.text(
          `${index + 1}. ${product.productName} (${product.company}) - ${product.totalQuantity} units`,
          25,
          125 + (index * 10)
        );
      });
  ...
  return (
    <div className="relative">
      <motion.button
        ...
        className="flex items-center gap-2 px-4 py-2 rounded-xl gradient-primary text-white font-semibold hover:shadow-lg transition-all duration-300 disabled:opacity-50"
      >
            ...
            >
                ...
                {exportOptions.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isExporting}
                      className="w-full text-left p-3 rounded-lg hover:bg-accent/50 transition-colors disabled:opacity-50 group"
                    >
              ...
              <div className="mt-4 pt-3 border-t border-border">
                <p className="text-xs text-muted-foreground">
                  Exporting {data.length} transactions from {format(dateRange.startDate, 'MMM dd')} to {format(dateRange.endDate, 'MMM dd, yyyy')}
                </p>
              </div>
            </motion.div>
          </>
