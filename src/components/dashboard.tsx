  ...
  return <DndProvider backend={HTML5Backend}>
      <div className="p-4 lg:p-8">
        ...
        y: 0
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-4xl font-bold"><span className="editable-text">Dashboard</span></h1>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setCustomizing(!isCustomizing)} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-semibold transition-all ${isCustomizing ? 'gradient-primary text-white shadow-lg' : 'glass-effect border border-border hover:bg-accent/50'}`}>
                {isCustomizing ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                {isCustomizing ? 'Exit Customize' : 'Customize'}
              </button>
              
              <button onClick={() => setShowCustomizer(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl glass-effect border border-border hover:bg-accent/50 transition-all">
                <Settings className="w-4 h-4" /><span className="editable-text">
                Widget Settings
              </span></button>
            </div>
          </div>
        ...
        y: 0
            ...
            <p className="text-blue-800 font-medium"><span className="editable-text">
        ...
        {/* Widgets Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {widgets.map((widget, index) => <DraggableWidget key={widget.id} widget={widget} index={index} onEdit={setEditingWidget} />)}
        </div>
        ...
        opacity: 1
            ...
            <button onClick={() => setShowCustomizer(true)} className="gradient-primary text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition-all"><span className="editable-text">
